...
public class DeviceDataManager implements IDataMessageListener
{
	...
	private void handleIncomingDataAnalysis(ResourceNameEnum resource, SensorData data)
	{
		...
		} else if (data.getTypeID() == ConfigConst.VIBRATION_SENSOR_TYPE) {
			this.handleVibrationSensorAnalysis(resource, data);
			handleUpstreamTransmission(resource, data, 0);
		}
	}
	...
	private void handleVibrationSensorAnalysis(ResourceNameEnum resource, SensorData data) {
		_Logger.info(String.format("Analyzing vibration data from CDA: %s. Value: %f", data.getLocationID(), data.getValue()));
		// Aquí puedes agregar lógica de análisis, alertas, etc.
	}
